...
///	インプットレイアウト
///	</summary>
const std::vector<D3D11_INPUT_ELEMENT_DESC> NormalFadeOut::INPUT_LAYOUT =
{
...
bool NormalFadeOut::Update(const float& deltaTime)
{
...
void NormalFadeOut::Enter()
{
	m_time = 0.0f;
}

void NormalFadeOut::Exit()
{
...
void NormalFadeOut::CreateShader(ID3D11Device1* device, ID3D11DeviceContext* context)
{

	auto shaderFactory = ShaderFactory::GetInstance();
	//各シェーダーの作成
	m_vertexShader = shaderFactory->CreateVS(device, "FadeVS.cso");
	m_pixelShader = shaderFactory->CreatePS(device, "NormalFadeOutPS.cso");
